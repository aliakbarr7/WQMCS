// Author       : Wenda Yusup dan Willy Rizqyan
...
void sensorDO()
{
  ...
  Serial.println("Sensor DO    : " + String((float(readDO(ADC_Voltage, Temperaturet)) / 1000)+3.17));
  // Serial.println( "DO :" +  ADC_Raw);
  // String(float((readDO(ADC_Voltage, Temperaturet)) / 1000)+2.35)
}
...
void sensorPH4502C()
{
  ...
  PH_step = (PH_4 - PH_7) / 3;
  Po = 7.00 + (((PH_7 - TeganganPh) / PH_step));

  
  // Jika semua sensor dicelupkan dalam air
  // Maka akan ada perubahan nilai pada raw Data Sensor pH
  // Sehingga nilai pembacaan pH dijumlah dengan nilai acuan dari alat calibrator
  if (Po < 2.8)
  { 
    Po = Po + 4.86;
...
// Sampling data sensor pH selama 5 kali, dan kirim rata - rata pembacaan
...
Task lcdUpdate(1500, []()
  ...
  if (millis() - LCD_Milis >= 2200) {
    ...
    if (LCD_Count == 0) {
      ...
      lcdPrint("O R P  :" + String ((int)orpppm) + "  ppm", "DO     :" + String((float(readDO(ADC_Voltage, Temperaturet)) / 1000)+3.17) + " ppm", "NO3-   :" + String (NO3Val) + " ppm", "pH     :" + String() + (Po) + " pH");
      LCD_Count = 3;
    }
    else if (LCD_Count == 3) {
